...
void app_main(void)
{
    ...
    {
        ESP_LOGE(MAIN_TAG, "Camera POST failed, exiting");
        return;
    }
    ...
    while(1)
    {
        ...
        ESP_LOGI(MAIN_TAG, "Image no: %i", count);
        ...
        jpg_image_t jpg_data = extract_camera_buffer(fb);
        ...
        const char img_file[FILENAME_MAX_SIZE];
        sprintf(img_file, MOUNT_POINT"/img%i.jpg", count);
        ...
        ESP_LOGI(MAIN_TAG, "Image no: %i complete", count++);
    }
}